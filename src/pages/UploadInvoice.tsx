...
export default function UploadInvoice() {
  ...
  const loadLibraries = useCallback(async () => {
    if (!(window as any).Tesseract) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        setTimeout(() => reject(new Error("Tesseract.js load timeout")), 12000);
      });
    }
    ...
    if (!(window as any).pdfjsLib) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        setTimeout(() => reject(new Error("PDF.js load timeout")), 12000);
      });
    }
  }, []);
  ...
  const extractTextFromPDF = useCallback(
    async (f: File) => {
      ...
      for (let i = 1; i <= pagesToRead; i++) {
        setOcrProgress(5 + Math.round((i / pagesToRead) * 45));
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const { data } = await workerRef.current.recognize(e.target?.result);
          setOcrProgress(100);
          resolve(data?.text || "");
        } catch (err) {
          reject(err);
  ...
  const processInvoice = async () => {
    ...
    try {
      ...
      const aiExtracted = await extractInvoiceDataFree(text, file.name);
      ...
      toast({ variant: "destructive", title: "Processing failed", description: e?.message || "Unknown error" });
    } finally {
      setUploading(false);
      setProcessing(false);
    }
  ...
  const fetchDriveFiles = async () => {
    if (!providerToken) {
      toast({
        ...
        description: "Logout and login again to refresh Drive permission.",
      });
      return;
    }
    ...
    try {
      setUploading(true);
      const data = await invokeEdge<{ files: any[] }>("drive-list", { providerToken });
      ...
      if (!finalFiles.length) {
        toast({
          ...
          description:
            "No PDF/image found. If they are in Shared Drive, this build supports it — check Google account + scopes.",
        });
      }
    } catch (e: any) {
      toast({ variant: "destructive", title: "Drive fetch error", description: e?.message || "Unknown error" });
    } finally {
      setUploading(false);
    }
  };
  ...
  const processSelectedDriveFile = async () => {
    ...
    if (!providerToken) {
      toast({ variant: "destructive", title: "Google token missing", description: "Please login again." });
      return;
    }
    ...
    try {
      const fileMeta = driveFiles.find((f) => f.id === selectedDriveFile);
      if (!fileMeta) throw new Error("Selected file not found");

      const data = await invokeEdge<{ base64: string; mimeType: string; filename: string }>("drive-download", {
        providerToken,
      ...
      const downloadedFile = new File([bytes], data.filename || fileMeta.name, {
        type: data.mimeType || fileMeta.mimeType,
      });
      ...
      const aiExtracted = await extractInvoiceDataFree(text, downloadedFile.name);
      ...
      setExtractedData(aiExtracted);
      toast({ title: "Processed!", description: "Invoice processed from Google Drive." });
    } catch (e: any) {
      setProcessingSteps((prev) => prev.map((s) => (s.status === "processing" ? { ...s, status: "error" } : s)));
      toast({ variant: "destructive", title: "Drive processing failed", description: e?.message || "Unknown error" });
    } finally {
      setUploading(false);
      setProcessing(false);
    }
  ...
  const fetchGmailInvoices = async () => {
    ...
    try {
      ...
      const data = await invokeEdge<{ messages: GmailMessage[] }>("gmail-list", {
        providerToken,
        maxResults: 20,
      });
      ...
      if (!msgs.length) {
        toast({
          variant: "destructive",
          title: "No invoice attachments found",
          description: "Try: search in Gmail for pdf invoices in the last 90 days.",
        });
      }
    } catch (e: any) {
      toast({ variant: "destructive", title: "Gmail error", description: e?.message || "Unknown error" });
    } finally {
  ...
  // ---------------------------
  // GMAIL: download attachment + process
  // ---------------------------
  const processSelectedGmailAttachment = async () => {
    ...
    if (!selectedGmailMsg || !selectedGmailAttachmentId) {
      toast({ variant: "destructive", title: "Select email + attachment first", description: "Pick an attachment." });
      return;
    }
    ...
    if (!msg || !att) {
      toast({ variant: "destructive", title: "Attachment not found", description: "Please select again." });
      return;
    }
    ...
    try {
      const data = await invokeEdge<{ base64: string; mimeType: string; filename: string }>(
      ...
      setExtractedData(aiExtracted);
      toast({ title: "Processed!", description: "Invoice processed from Gmail." });
    } catch (e: any) {
      setProcessingSteps((prev) => prev.map((s) => (s.status === "processing" ? { ...s, status: "error" } : s)));
      toast({ variant: "destructive", title: "Gmail processing failed", description: e?.message || "Unknown error" });
    } finally {
      setUploading(false);
      setProcessing(false);
    }
  ...
  const saveInvoice = async () => {
    try {
      ...
      if (!file || !extractedData) {
        toast({ variant: "destructive", title: "Missing data", description: "No file or extracted data." });
        return;
      }
      ...
      let { error: insErr } = await supabase.from("invoices").insert(payloadWithStorage);

      if (insErr && /column .* does not exist/i.test(insErr.message || "")) {
        const retry = await supabase.from("invoices").insert(basePayload);
        insErr = retry.error;
      }
