...
export default function UploadInvoice() {
  ...
  // ✅ Google Login (forces account chooser)
  const handleGoogleSignIn = async () => {
    // IMPORTANT: redirect back to SAME PAGE to avoid 404 after login
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      ...
      options: {
        redirectTo,
        // These scopes help provider_token include access to Drive/Gmail (if your provider settings allow it)
        scopes:
          "openid email profile https://www.googleapis.com/auth/drive.readonly https://www.googleapis.com/auth/gmail.readonly",
        queryParams: {
          access_type: "offline",
          prompt: "select_account consent", // ✅ show account chooser always
        },
      },
    });
  ...
  const invokeFn = async (name: string, body: any) => {
    ...
    if (error) {
      ...
      const message = error?.message || "Edge Function returned a non-2xx status code";
  ...
  const fetchDriveFiles = async () => {
    ...
    if (!providerToken) {
      toast({
        ...
        description: 'Click "Sign in with Google" again (select_account) to refresh tokens.',
      });
      return;
    }
    ...
    try {
      ...
      const data = await invokeFn("drive-list", { providerToken });

      // Normalize possible return formats:
      // - { files: [...] }
      // - { files: { files: [...] } }
      const raw = (data?.files?.files ?? data?.files ?? []) as any[];
      const finalFiles: DriveFile[] = Array.isArray(raw)
        ? raw.map((f: any) => ({
            id: f.id,
            name: f.name,
            mimeType: f.mimeType,
      ...
      if (finalFiles.length === 0) {
        toast({
          ...
          description:
            "No PDF or image files found. If your files are in Shared Drive, your drive-list function must use includeItemsFromAllDrives/supportsAllDrives.",
        });
      }
    } catch (error: any) {
  ...
  const processSelectedDriveFile = async () => {
    ...
    if (!providerToken) {
      toast({
        variant: "destructive",
        title: "Google token missing",
        description: "Logout and login again with Drive permission.",
      });
      return;
    }
    ...
    try {
      ...
      const data = await invokeFn("drive-download", { providerToken, fileId: selectedDriveFile });
  ...
  const fetchGmailInvoices = async () => {
    if (!providerToken) {
      toast({
        variant: "destructive",
        title: "Google token missing",
        description: "Logout and login again with Gmail permission.",
      });
      return;
    }
    ...
    try {
      ...
      const data = await invokeFn("gmail-list", { providerToken, maxResults: 20 });
      ...
      if (!msgs.length) {
        toast({
          ...
          description:
            "No invoice attachments found in Gmail (last 90 days). Try having PDF invoices in INBOX or adjust gmail-list query.",
        });
      }
    } catch (e: any) {
                ...
                {!isAuthenticated ? (
                  ...
                  <>
                    ...
                    {!providerToken && (
                      <Alert>
                        <AlertDescription>
                          Your Google token is missing (provider_token is null). Click Google sign-in again and choose
                          account.
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    {driveFiles.length === 0 ? (
                      <div className="text-center py-6">
                        ...
                        <p className="text-xs text-gray-500 mt-3">
                          If you use Shared Drive, your drive-list Edge Function must support AllDrives.
                        </p>
                      </div>
                    ) : (
