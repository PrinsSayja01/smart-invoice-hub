...
export default function UploadInvoice() {
  ...
  // ✅ IMPORTANT: refresh Supabase session so Edge Functions won't return 401
  const ensureFreshSession = useCallback(async () => {
    ...
    if (s.expires_at && s.expires_at - now < 60) {
      ...
      if (refreshed.data.session) {
        ...
        setProviderToken(refreshed.data.session.provider_token || null);
  ...
  // ✅ Google Login (forces account chooser + consent every time)
  const googleSignIn = useCallback(
    async (forceAccountChooser: boolean) => {
      const redirectTo = window.location.origin + "/dashboard/upload";

      // If user wants other account, signOut first so Google won't silently reuse existing session.
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          queryParams: {
            access_type: "offline",
            prompt: forceAccountChooser ? "consent select_account" : "consent",
  ...
  const handleGoogleSignIn = async () => googleSignIn(false);
  const handleUseAnotherAccount = async () => googleSignIn(true);
  ...
  const extractTextFromPDF = useCallback(
    async (f: File) => {
      ...
      for (let i = 1; i <= pages; i++) {
        setOcrProgress(10 + Math.round((i / pages) * 40));
  ...
  const processInvoice = async () => {
    ...
    try {
      ...
      alert(`Processing failed: ${corsSafeError(e)}`);
    } finally {
      setUploading(false);
      setProcessing(false);
    }
  ...
  const fetchDriveFiles = async () => {
    ...
    if (!token) {
      alert("Google token missing. Click 'Use another account' and login again (consent screen must show Drive/Gmail).");
      return;
    }
    ...
    try {
      ...
      if (!finalFiles.length) {
        alert(
          "No invoice files found.\n\nTip: Ensure files are PDF/JPG/PNG and not Google Docs. Also ensure you accepted Drive permission on login."
        );
      }
    } catch (e: any) {
      console.error(e);
      alert(`Error fetching files: ${corsSafeError(e)}`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const processSelectedDriveFile = async () => {
    ...
    if (!token) return alert("Google token missing. Use 'Use another account' and login again.");
    ...
    try {
      ...
      if (!meta) throw new Error("Selected file not found");

      const { data, error } = await supabase.functions.invoke("drive-download", {
        body: { providerToken: token, fileId: selectedDriveFile, mimeType: meta.mimeType, filename: meta.name },
      });
      if (error) throw error;
      ...
      const downloadedFile = new File([bytes], data.filename || meta.name, {
        type: data.mimeType || meta.mimeType,
      });
  ...
  const fetchGmailInvoices = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("gmail-list", {
        body: { providerToken: token, days: 90, maxResults: 20 },
      });
      if (error) throw error;
  ...
  const processSelectedGmailAttachment = async () => {
    ...
    if (!s) return alert("Please login first.");
    if (!token) return alert("Google token missing. Use 'Use another account' and login again.");
    if (!selectedGmailMsg || !selectedGmailAttachmentId) return alert("Select an email and attachment first.");
    ...
    if (!msg || !att) return alert("Attachment not found.");
  ...
  // ✅ Save: storage upload + insert row (fixes file_url not-null)
  const saveInvoice = async () => {
    try {
      if (!isAuthenticated) return alert("Please login first.");
      if (!file || !extractedData) return alert("Missing file or extracted data.");
      ...
      // IMPORTANT: if your invoices table has file_url NOT NULL, we must send a value
      const fileUrlFinal = publicUrl || `storage:${storagePath}`;
      ...
      const { error: insErr } = await supabase.from("invoices").insert(basePayload);
