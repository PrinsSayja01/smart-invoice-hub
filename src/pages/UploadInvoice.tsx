...
export default function UploadInvoice() {
  ...
  const handleGoogleSignIn = async () => {
    const redirectTo = window.location.origin + window.location.pathname; // ✅ stay on same page (no 404)
  ...
  const extractTextFromPDF = useCallback(
    async (f: File) => {
      ...
      for (let i = 1; i <= pagesToRead; i++) {
        setOcrProgress(10 + Math.round((i / pagesToRead) * 30));
  ...
  // ✅ Camera select (mobile will open camera)
  const handleCameraCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    const selected = e.target.files?.[0];
    if (selected && isValidInvoiceFile(selected)) {
      setFile(selected);
      setExtractedData(null);
      toast({ title: "Captured", description: "Camera image selected." });
    } else {
      toast({ variant: "destructive", title: "Invalid capture", description: "Only JPG/PNG/PDF allowed." });
    }
  };
    ...
    try {
      ...
      if (!finalFiles.length) {
        toast({
          ...
          description:
            "No PDF or image files found. If your invoices are in Shared Drive, they should appear now (supportsAllDrives enabled).",
        });
      }
    } catch (error: any) {
    ...
    try {
      ...
      if (!msgs.length) {
        toast({
          ...
          description:
            "No invoice attachments found in Gmail (last 90 days). Try receiving pdf/jpg invoices in Inbox.",
        });
      }
    } catch (e: any) {
                  ...
                  {file ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2 justify-center">
                        ...
                        <Button asChild variant="outline">
                          <label htmlFor="camera-upload" className="cursor-pointer flex items-center">
