...
function b64ToFile(base64: string, name: string, mimeType: string) {
...
export default function UploadInvoice() {
  ...
  const providerToken = session?.provider_token; // ✅ Google OAuth access token (ya29...)
  ...
  const handleGoogleSignIn = async () => {
    await supabase.auth.signInWithOAuth({
      ...
      options: {
        ...
        // ✅ Drive + Gmail scopes (you can remove gmail.readonly if not needed)
  ...
  const fetchDriveFiles = async () => {
    if (!providerToken) {
      alert('Google token missing. Logout and login again (with consent).');
      return;
    }
    ...
    try {
      ...
      if (!files.length) {
        alert('No PDF or image files found in your Google Drive (root). Upload a PDF/image first, then press Refresh.');
      }
    } finally {
      setUploading(false);
    }
  ...
  const processSelectedDriveFile = async () => {
    ...
    if (!providerToken) {
      alert('Google token missing. Logout and login again.');
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(`${error.message} ${(error as any).context?.details ?? ''}`);
      }

      const downloadedFile = b64ToFile((data as any).base64, meta.name, meta.mimeType);
      ...
      const invoice = await extractWithEdge(text, downloadedFile);
  ...
  const extractWithEdge = async (text: string, f: File): Promise<ExtractedData> => {
    const { data, error } = await supabase.functions.invoke('process-invoice', {
      body: { fileName: f.name, fileType: f.type, extractedText: text },
    });
    ...
    return {
      ...
      invoice_type: r.invoice_type ?? 'other',
      risk_score: r.risk_score ?? 'low',
      compliance_status: r.compliance_status ?? 'compliant',
  ...
  const processInvoice = async () => {
    ...
    try {
      ...
      const invoice = await extractWithEdge(text, file);
      setExtractedData(invoice);
  ...
  const saveInvoice = async () => {
    if (!file || !extractedData || !session?.user?.id) return;
    ...
    try {
      ...
      // ✅ 2) get public url (or use signed urls later)
      ...
      const { error: dbErr } = await supabase.from('invoices').insert({
        ...
        compliance_status: extractedData.compliance_status || 'compliant',
          ...
          <TabsContent value="drive">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Browse PDF / image invoices from your Drive</CardDescription>
              </CardHeader>
                  ...
                  <>
                    <Alert>
                      <AlertDescription>
                        ...
                        <br />
                        Provider token: <strong>{providerToken ? '✅ present' : '❌ missing (re-login with consent)'}</strong>
                      </AlertDescription>
                    </Alert>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm mb-2">Gmail integration will be added later.</p>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
        ...
        {processingSteps.length > 0 && !extractedData && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>OCR + extraction</CardDescription>
            </CardHeader>
        ...
        {extractedData && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Verify before saving</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
